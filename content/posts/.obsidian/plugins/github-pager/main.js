...
var GitHubPagerSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian.Setting(containerEl).setName("GitHub token").setDesc("The personal access token with repo scope. See https://docs.github.com/en/github/authenticating-to-github/creating-a-personal-access-token").addText((text) => text.setPlaceholder("It should be something like `ghp_...`").setValue(this.plugin.settings.githubToken).onChange(async (value) => {
      this.plugin.settings.githubToken = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Repository owner").setDesc("The owner of the repository (user or org), like: octocat").addText((text) => text.setValue(this.plugin.settings.repositoryOwner).onChange(async (value) => {
      this.plugin.settings.repositoryOwner = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Repository name").setDesc("The name of the repository, like: hello-world").addText((text) => text.setValue(this.plugin.settings.repositoryName).onChange(async (value) => {
      this.plugin.settings.repositoryName = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Base path").setDesc("Path in the repo where notes will be saved (e.g. content/posts).").addText((text) => text.setPlaceholder("The directory path in the remote repo, e.g. content/posts").setValue(this.plugin.settings.basePath).onChange(async (value) => {
      this.plugin.settings.basePath = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Image path").setDesc("Path in the repo where images will be saved.").addText((text) => text.setPlaceholder("The directory path in the remote repo for images, e.g. static/images").setValue(this.plugin.settings.imagePath).onChange(async (value) => {
      this.plugin.settings.imagePath = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Commit message").setDesc("Commit message template. Use {{file}} as a placeholder for the filename.").addText((text) => text.setPlaceholder("Update {{file}} via Obsidian").setValue(this.plugin.settings.commitMessage).onChange(async (value) => {
      this.plugin.settings.commitMessage = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Auto sync").setDesc('Automatically push changes to GitHub when you save a file with the "share" frontmatter property set to true.').addToggle((toggle) => toggle.setValue(this.plugin.settings.autoSync).onChange(async (value) => {
...
function throttling(octokit, octokitOptions) {
  ...
  state.retryLimiter.on("failed", async function(error, info) {
    ...
    const { wantRetry, retryAfter = 0 } = await async function() {
      ...
      if (error.response.headers != null && error.response.headers["x-ratelimit-remaining"] === "0" || ((_b = (_a = error.response.data) == null ? void 0 : _a.errors) != null ? _b : []).some(
...
var MyOctokit = Octokit2.plugin(retry, throttling);
var GitHubAdapter = class {
  ...
  async pushFile(path, contentBase64, message) {
    try {
      let sha;
      try {
...
var ContentProcessor = class {
  constructor(adapter, app, settings) {
  ...
  async process(file) {
    ...
    for (const match of linkMatches) {
      ...
      if (linkedFile) {
        const linkedFrontmatter = (_a = this.app.metadataCache.getFileCache(linkedFile)) == null ? void 0 : _a.frontmatter;
        let basePath = this.settings.basePath;
        if ((linkedFrontmatter == null ? void 0 : linkedFrontmatter.remote_path) && typeof linkedFrontmatter.remote_path === "string") {
  ...
  async uploadImage(file) {
    ...
    const contentBase64 = (0, import_obsidian2.arrayBufferToBase64)(content);
...
// src/main.ts
var GitHubPagerPlugin = class extends import_obsidian3.Plugin {
  constructor() {
    super(...arguments);
    ...
    this.addCommand({
      ...
      callback: async () => {
        if (!this.githubAdapter) {
          new import_obsidian3.Notice("GitHub adapter not initialized. Check settings.");
          return;
        }
        const user = await this.githubAdapter.verifyAuth();
        if (user) {
          new import_obsidian3.Notice(`Authenticated as ${user}`);
        } else {
          new import_obsidian3.Notice("Authentication failed. Check token.");
        }
      }
    });
    this.addCommand({
      ...
      callback: async () => {
        ...
        if (!this.githubAdapter || !this.processor) {
          new import_obsidian3.Notice("GitHub adapter not ready.");
          return;
        }
        try {
          ...
          console.error(e);
          new import_obsidian3.Notice("Error pushing file.");
        }
      }
    });
    this.registerEvent(
      this.app.workspace.on("file-menu", (menu, file) => {
        if (file instanceof import_obsidian3.TFile) {
          menu.addItem((item) => {
            item.setTitle("Sync to GitHub").setIcon("github").onClick(async () => {
              await this.pushFile(file);
            });
          });
        }
  ...
  async pushFile(file) {
    ...
    const contentBase64 = (0, import_obsidian3.arrayBufferToBase64)(data.buffer);
    const frontmatter = (_a = this.app.metadataCache.getFileCache(file)) == null ? void 0 : _a.frontmatter;
    const remotePathOverride = frontmatter == null ? void 0 : frontmatter.remote_path;
    let basePath = this.settings.basePath;
    ...
    new import_obsidian3.Notice(`Pushing ${file.name}...`);
    const success = await this.githubAdapter.pushFile(path, contentBase64, message);
    if (success) {
      new import_obsidian3.Notice(`Successfully pushed ${file.name}`);
    } else {
      new import_obsidian3.Notice(`Failed to push ${file.name}`);
    }
    return success;
  }
  onunload() {
